...
class AtivoController:
    ...
    def editar_ativo(self, tabela, dados):
        conn = get_connection()
        cursor = conn.cursor()

        cursor.execute(f"""
            UPDATE "{tabela}"
            SET tipo=?, nome=?, especificacoes=?, valor=?, estado=?, data_aquisicao=?, vida_util=?, status=?
            WHERE id=?
        """, (
            dados["Tipo"],
            dados["Nome"],
            ...
            float(dados["Valor"]),
            dados["Estado"],
            dados["Data de Aquisição"],
            int(dados["Vida Útil (em anos)"]),
            dados["Ativo/Inativo"],
            dados["ID"]
        ))
    ...
    def deletar_ativo(self, tabela, id_ativo):
        conn = get_connection()
        cursor = conn.cursor()

        cursor.execute(f'DELETE FROM "{tabela}" WHERE id = ?', (id_ativo,))
