...
class MainView:
    def __init__(self, root):
        ...
        tk.Button(root, text="Editar Ativo", command=self.editar_ativo).grid(row=1, column=0, columnspan=2, pady=5)
        # Botão para deletar ativos
        tk.Button(root, text="Deletar Ativo", command=self.deletar_ativo).grid(row=2, column=0, columnspan=2, pady=5)

        tk.Button(root, text="Gerar Relatório", command=self.gerar_relatorio).grid(row=3, column=0, columnspan=2, pady=5)
