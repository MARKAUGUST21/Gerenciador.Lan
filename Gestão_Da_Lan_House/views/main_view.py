...
class MainView:
    def __init__(self, root):
        ...
        tk.Button(root, text="Adicionar Ativo", command=self.abrir_janela_adicionar).grid(row=0, column=0, columnspan=2, pady=5)

        # Notebook com abas por tipo de ativo
        self.notebook = ttk.Notebook(root)
        self.notebook.grid(row=5, column=0, columnspan=2, pady=10)
        ...
        def confirmar():
            try:
                ...
                if ativo_escolhido in ("Computador", "Monitor", "Periférico"):
                    tipo = tipo_var.get()
                    ativo = ClasseSelecionada(id, tipo, nome, espec, valor, data_formatada, estado, vida, status)
                else:
                    ativo = ClasseSelecionada(id, nome, espec, valor, data_formatada, estado, vida, status)
                self.controller.adicionar_ativo(ativo)

                nome_tabela_destino = self.tabela[ClasseSelecionada]
                self.atualizar_tabela(nome_tabela_destino)
    ...
    def atualizar_tabela(self, tabela):
        ...
        classes_dessa_tabela = [cls for cls, nome in self.tabela.items() if nome == tabela]

        for ativo in self.controller.listar_ativos():
            if any(isinstance(ativo, cls) for cls in classes_dessa_tabela):
                valor_formatado = f"R$ {float(ativo.valor):,.2f}".replace(',', 'v').replace('.', ',').replace('v', '.')
                tree.insert("", tk.END, values=(ativo.id, ativo.tipo, ativo.nome, ativo.especificacoes, valor_formatado, ativo.estado, ativo.data_aquisicao, ativo.vida_util, ativo.status))